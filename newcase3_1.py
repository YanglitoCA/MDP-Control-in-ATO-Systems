...
def run_two_product_model(
    ...
    total_due = np.zeros(2, int)
    total_satisfied_due = np.zeros(2, int)
    ...
    print(f"Total Satisfied Due:  {total_satisfied_due}")
...
def plot_production_threshold(zetaP: np.ndarray):
    ...
    ax.set_xlabel('Backlog B1')
    ax.set_ylabel('Backlog B2')
    ax.set_zlabel('ζ0^P')
...
if __name__ == "__main__":
    ...
    init_state = np.array([4, 2, 1])  # ensure b0 ≤ prod_capacity
